...
def configure_logging(out_dir, tomod_id):
    ...
    log_path = os.path.join(log_dir, f"simulation_{job_id}_{tomod_id}.log")
    error_log_path = os.path.join(log_dir, f"simulation_{job_id}_{tomod_id}_error.log")
    ...
    logger.info(f"Logging initialized for tomogram {tomod_id}.")
...
def generate_tomogram(tomod_id, global_params):
    ...
    configure_logging(OUT_DIR, tomod_id)
...
def main():
    ...
    global_params = (
        ...
        MMER_TRIES, PMER_TRIES, SEED, MEMBRANES_LIST, HELIX_LIST, PROTEINS_LIST, MB_PROTEINS_LIST,
        PROP_LIST, PMER_OCC_LIST, SURF_DEC, MT_PMER_OCC, ACTIN_PMER_OCC, USE_PMER_OCC_LIST, 
        LBL_MB, LBL_AC, LBL_MT, LBL_CP, LBL_MP
    )
